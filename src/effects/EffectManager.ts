...
export class EffectManager {
  ...
  private readonly MAX_DECORATIONS_PER_TYPE = 1;

  constructor(context: vscode.ExtensionContext, settings: SettingsService) {
  ...
  private getTtl(kind: EffectKind): number {
    return kind === "boom"
      ? this.settings.data.booms.duration
      : kind === "blip"
      ? this.settings.data.blips.duration
      : this.settings.data.slams.duration;
  }
  ...
  private ensureAnimating(editor: vscode.TextEditor, kind: EffectKind) {
    ...
    const tick = () => {
      ...
      const extraY = this.settings.data.keys.floatDistance; // additional float up over lifetime
      const baseScale = this.settings.data.keys.scaleBase;
      const extraScale = this.settings.data.keys.scaleAdd;
      ...
      for (const item of buf) {
        ...
        after.textDecoration = `none; position: absolute; display: inline-block; transform: translate(-50%, ${y}em) scale(${s}); transform-origin: left bottom; pointer-events: none; z-index: 1000; line-height: 0;`;
      }
      ...
      state.animTimers[kind] = setTimeout(
        tick,
        this.settings.data.general.updateRate,
      );
    };
  ...
  private cleanupInvisibleEditors(
  ...
  private canAddDecoration(
    ...
    const state = this.getEditorState(editor);
    let maxDecorations = this.settings.data.general.maxDecorationCount;
    return state.activeDecorations[kind] < maxDecorations;
  }
  ...
  private async ensureSpriteData(kind: EffectKind): Promise<void> {
    ...
    (this.spriteData as Record<EffectKind, any>)[kind] = {
      frames,
      sheetW,
      sheetH,
      fps,
  ...
  private async playSpriteAnim(editor: vscode.TextEditor, kind: EffectKind) {
    ...
    const step = () => {
      ...
      if (kind === "boom") {
        ...
        const targetHeightPx = fontPx * 3; // exactly 1em (same as font size)
        ...
        const tyEm = lineEm / 2 - heightEm / 2; // center vertically in line
        ...
        const svg = `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${wPx} ${hPx}" width="${targetWidthPx}" height="${targetHeightPx}">\n  <image href="data:image/png;base64,${data.pngBase64}" x="-${frame.x}" y="-${frame.y}" width="${sheetW}" height="${sheetH}" preserveAspectRatio="none" style="image-rendering:pixelated"/>\n</svg>`;
        ...
        opt = {
          ...
          renderOptions: {
            after: {
              ...
              width: "0",
              textDecoration: `none; position:absolute; display:inline-block; line-height:0; transform: translate(${txEm.toFixed(
                3
              )}em, ${tyEm.toFixed(
                3
              )}em); transform-origin:left bottom; pointer-events:none; z-index:1000;`,
            },
          },
        };
      } else if (kind === "blip") {
        ...
        const wPx = Math.max(1, Math.round(frame?.w ?? 1));
        const fontPx = this.getEditorFontSizePx(editor);
        const targetHeightPx = fontPx * (3 + idx * 0.5); // exactly 1em
        const targetWidthPx = Math.max(
          1,
          Math.round((wPx / hPx) * targetHeightPx)
        );
  ...
  private applyOnce(
        ...
        return {
          ...
          renderOptions: {
            after: {
              ...
              height: "1em",
              width: "0",
              textDecoration: `none; position: absolute; display: inline-block; line-height: 0; transform: translate(-50%, -1.1em) scale(${this.settings.data.keys.scaleBase}); transform-origin: left bottom; pointer-events: none; z-index: 1000;`,
            },
          },
    ...
    const cap = Math.max(this.settings.data.keys.maxCount, this.MAX_DECORATIONS_PER_TYPE);
  ...
  private shake(editor: vscode.TextEditor, extendMs: number) {
    const state = this.getEditorState(editor);
    const now = Date.now();
    const decayMs = Math.max(20, this.settings.data.shakes.duration);
    ...
    const amplitudePx = Math.max(
      0,
      Math.min(32, this.settings.data.shakes.intensity)
    );
